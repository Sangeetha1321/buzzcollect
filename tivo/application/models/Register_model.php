...
class Register_Model extends CI_Model 
{
	...
	public function unitpricedata($data) 
	{
		$res = $this->db->insert_batch("project_task_unitprice", $data);
		if($res){
            return TRUE;
        }else{
            return FALSE;
	...
	public function get_category_status($id) 
	{
		return $this->db->where("ID", $id)->get("member_categories");
	}
	
	public function check_employee_id_is_free($employee_id) 
	{
		$s=$this->db->where("employee_id", $employee_id)->get("users");
		if($s->num_rows() > 0) {
			return false;
		} else {
			return true;
	...
	{
		$s=$this->db->where("email", $email)->where("email !=","")->get("users");
		if ($s->num_rows() > 0) {
			return false;
		} else {
			return true;
