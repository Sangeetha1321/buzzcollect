		...
		<?php endif; ?>
    		...
    		<?php echo form_open(site_url("register"), array("id" => "register_form")) ?>
				<div class="form-group login-form-area has-feedback">
                    <input type="text" class="form-control required" name="employee_id" placeholder="<?php echo lang("ctn_1571") ?>" id="employee-id" value="<?php if(isset($employee_id)) echo $employee_id; ?>" maxlength="6">
...
<script type="text/javascript">
  $(document).ready(function() {
	...
	if( !$("#email").val() ) {
	var email = $("#email").val("<?php echo md5(uniqid(rand(1000,10000), true)); ?>@spi-global.com");
	...
	$('#employee-id').on("change", function() {
    	$.ajax({
	        url : global_base_url + "register/check_employee_id",
	        type : 'GET',
	        data : {
	        ...
	        success: function(data) {
	        	if(data.success) {
	        		$("#login-icon-employee-id").css("color", "green");
	        	} else {
	        		$("#login-icon-employee-id").css("color", "#a0a0a0");
