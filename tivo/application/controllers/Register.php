...
class Register extends CI_Controller 
{
	...
	public function index()
	{
		...
		if (isset($_POST['username']) || isset($_POST['username'])) {
			$employee_id = $this->common->nohtml(
	...
	{
		$employee_id = $this->common->nohtml($this->input->get("employee_id", true));
		$field_errors = array();

		/* if (!preg_match("/^[a-z0-9_]+$/i", $employee_id)) {
			$field_errors['employee_id'] = lang("error_299");
		} */
		if (!$this->register_model->check_employee_id_is_free($employee_id)) {
			$field_errors['employee_id'] = "$employee_id " . lang("error_298");
		}
