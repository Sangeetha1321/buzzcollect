...
class Tasks extends CI_Controller 
{
	...
	public function tasks_page($page = "index", $projectid=0, $u_status =0) 
	{
		...
		foreach($tasks->result() as $r) {
			if($r->status == 1) {
				...
				$status = "<label class='label label-success'>".lang("ctn_1633")."</label>";
			} elseif($r->status == 7) {
				$status = "<label class='label label-danger'>".lang("ctn_1634")."</label>";
			} elseif($r->status == 8) {
				$status = "<label class='label label-success'>".lang("ctn_1635")."</label>";
			}
	...
	public function add_task_process() 
	{
		...
		$chapterwise = intval($this->input->post("chapterwise"));
		if($chapterwise == 1) {
			$chaptername = $this->lib_filter->go($this->input->post("chaptername"));
		} else{
			$chaptername = "";
			$chapterwise = 0;
		}
	...
	public function view($taskid, $page=0) 
	{
		...
		$project_tasks_chaptername = $this->task_model->get_project_tasks_chaptername($taskid);
		...
		$this->template->loadContent("tasks/view_task.php", array(
			...
			"project_tasks_chaptername" => $project_tasks_chaptername,
			"tasks" => $tasks
			)
		);
	...
	public function add_task_objective($taskid) 
	{
		...
		$chapternom = $this->input->post("chapt_list");
		$chapt_list = implode(",", $chapternom);
		...
		$this->user_model->add_user_log(array(
			...
			"</b> ".lang("ctn_1058")." <a href='".site_url("tasks/view/" . $task->ID)."'>" . $task->name . "</a>",
			"timestamp" => time(),
			"IP" => $_SERVER['REMOTE_ADDR'],
			"projectid" => $task->projectid,
			"url" => "tasks/view/" . $task->ID,
	...
	public function edit_objective($id) 
	{
		...
		$project_tasks_chaptername = $this->task_model->get_project_tasks_chaptername($objective->taskid);
	...
	public function add_file($taskid) 
	{
		...
		$update_status = $this->task_model->update_task_status($taskid);
