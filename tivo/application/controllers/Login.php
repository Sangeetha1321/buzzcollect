...
class Login extends CI_Controller 
{
	...
	public function ajax_check_login() 
	{
		...
		$field_errors = array();
		if(empty($data['email'])) {
			$field_errors['email'] = lang("error_254");
		}
		if(empty($data['pass'])) {
			$field_errors['pass'] = lang("error_281");
		}
		...
		if ($login->num_rows() == 0) {
			...
			if($login->num_rows() == 0) {
			$login = $this->login_model->getUserByEmployeeId($email);	
				if($login->num_rows() == 0) {
    	...
    	{
    		if(!$r->active) {
    			$this->template->jsonError(lang("error_133") . " <a href='".
	...
	{	
		...
		$userid = $r->ID;
		$email = $r->email;
		/* if(!empty($email)){echo $pass.'1pass'; echo $email.'1email'; exit;}else{echo $pass.'2pass'; echo $email.'2email'; exit;}
		 */
    	...
    	{
    		if(!$r->active) {
    			$this->template->error(lang("error_133") . " <a href='".site_url("register/send_activation_code/" . $r->ID . "/" .
    				 urlencode($r->email))."'>".lang("ctn_1022")."</a> " . lang("error_134"));
    		}
    	}
		...
		if($email == ''){
			$mail = "v.sangeetha@spi-global.com";
			/* $token = "94ad7977b0e46f28b2c39175537f9871"; */
			if($this->settings->info->secure_login) {
				// Generate a token
				$token = rand(1,100000) . $mail;
				$token = md5(sha1($token));
				// Store it
				$this->login_model->updateUserToken($userid, $token);
				/* echo "s1"; */
			...
			setcookie($config . "un", $mail, time()+$ttl, "/"); 
			setcookie($config . "tkn", $token, time()+$ttl, "/");  
			/* echo $mail.'=>'.$token.'=>'.$userid;
			exit;  */   
		}else{
